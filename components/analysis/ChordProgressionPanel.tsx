...
import { ChordEvent, TrackAnalysisData } from '../../types';
import { detectChordsAttack, detectChordsSustain, detectChordsHybrid, detectChordsBucketed } from '../services/midiHarmony';
...
export default function ChordProgressionPanel({ data, onRecalculationComplete }: ChordProgressionPanelProps) {
    const [chordDetectionMode, setChordDetectionMode] = useState<'sustain' | 'attack' | 'hybrid' | 'beat_synced'>('attack');
    ...
    const [bucketSizeBeats, setBucketSizeBeats] = useState<number>(1);
    const [voiceConfigs, setVoiceConfigs] = useState<Record<number, string>>({});
    const [arpeggioMode, setArpeggioMode] = useState<'count' | 'beat' | '2beat'>('count');
    ...
    useEffect(() => {
        ...
        if (chordDetectionMode === 'attack') {
            ...
            newChords = detectChordsHybrid(data.notesRaw, data.ppq, data.timeSignature.numerator, data.timeSignature.denominator, minDurTicks, voiceConfigs, arpeggioMode, arpeggioCount);
        } else if (chordDetectionMode === 'beat_synced') {
            newChords = detectChordsBucketed(data.notesRaw, data.ppq, data.timeSignature.numerator, data.timeSignature.denominator, bucketSizeBeats);
        }
        
        setVisibleChords(newChords);
        onRecalculationComplete(chordDetectionMode, newChords);
    }, [toleranceIndex, minNoteDurationIndex, chordDetectionMode, data.notesRaw, bucketSizeBeats, voiceConfigs, arpeggioMode, arpeggioCount]);
    ...
    return (
        <div className="mt-8 border-t border-gray-700 pt-6">
                 ...
                 <div className="flex flex-col gap-3 items-end w-full sm:w-auto">
                     <div className="flex flex-col gap-2 w-full">
                         ...
                         <div className="flex bg-gray-800 rounded-lg p-1 border border-gray-700 self-end flex-wrap gap-1 justify-end">
                            {['sustain', 'attack', 'beat_synced', 'hybrid'].map(mode => (
                                <button key={mode} onClick={() => setChordDetectionMode(mode as any)} className={`px-3 py-1 text-xs font-medium rounded-md transition-colors ${chordDetectionMode === mode ? 'bg-brand-primary text-white shadow' : 'text-gray-400 hover:text-white'}`}>
                                    {mode === 'beat_synced' ? 'Beat Sync' : mode.charAt(0).toUpperCase() + mode.slice(1).replace('_',' ')}
                                </button>
                            ))}
                         </div>
                     </div>
            ...
            {/* Beat Synced Config */}
            {chordDetectionMode === 'beat_synced' && (
                <div className="mb-6 p-4 bg-gray-800/50 rounded-lg border border-gray-700 animate-fade-in">
                     <h4 className="text-sm font-bold text-gray-300 mb-3 border-b border-gray-700 pb-1">Harmonic Rhythm Normalization</h4>
                    <div className="flex gap-4">
                        {[1, 2, 4].map(beat => (
                            <label key={beat} className="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="bucket" checked={bucketSizeBeats === beat} onChange={() => setBucketSizeBeats(beat)} className="text-brand-primary" />
                                <span className="text-sm text-gray-300">{beat === 4 ? '1 Measure' : `${beat} Beat${beat>1?'s':''}`}</span>
                            </label>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                         {Array.from({ length: data.voiceCount }).map((_, i) => (
                             <div key={i} className="flex items-center gap-2 p-2 bg-gray-800 rounded border border-gray-700">
                                 ...
                                 <select value={voiceConfigs[i] || 'sustain'} onChange={(e) => setVoiceConfigs(prev => ({...prev, [i]: e.target.value}))} className="bg-gray-900 border border-gray-600 text-xs text-gray-200 rounded py-1 px-2 focus:ring-brand-primary focus:border-brand-primary">
