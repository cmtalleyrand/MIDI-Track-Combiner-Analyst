...
export default function QuantizationSettings({
    ...
    pruneOverlaps, setPruneOverlaps,
    pruneThresholdIndex, setPruneThresholdIndex,
    quantizationWarning
    ...
    return (
        <div className="border-t border-gray-medium pt-4">
            <h3 className="text-lg font-semibold text-gray-light mb-4">Shadow Grid (Quantization)</h3>
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                    ...
                    <div className="md:col-span-2 p-4 bg-gray-darker rounded-lg border border-gray-medium">
                        <label className="flex items-center cursor-pointer mb-3">
                            <input type="checkbox" checked={pruneOverlaps} onChange={(e) => setPruneOverlaps(e.target.checked)} className="h-5 w-5 rounded bg-gray-dark border-gray-medium text-brand-primary focus:ring-brand-primary focus:ring-2" />
                            <div className="ml-3">
                                <span className="font-semibold text-gray-light">Prune Pitch Overlaps</span>
                                <p className="text-xs text-gray-400">Ensures monophony per pitch (clean sheet music).</p>
                            </div>
                        </label>
                        {pruneOverlaps && (
                            <div className="flex items-center gap-4 mt-4 border-t border-gray-700 pt-3">
                                <span className="text-xs text-gray-400">Min Duration:</span>
                                <input
                                    type="range" min="1" max={MUSICAL_TIME_OPTIONS.length - 1} step="1"
                                    value={pruneThresholdIndex} onChange={(e) => setPruneThresholdIndex(Number(e.target.value))}
                                    className="flex-grow h-2 bg-gray-600 rounded-lg appearance-none cursor-pointer accent-brand-primary"
                                />
                                <span className="text-xs font-mono text-brand-primary w-12 text-right">{MUSICAL_TIME_OPTIONS[pruneThresholdIndex].label}</span>
                            </div>
                        )}
                    </div>
                </div>
